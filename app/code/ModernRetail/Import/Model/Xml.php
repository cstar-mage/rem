...
class Xml extends \Magento\Framework\DataObject {
    ...
    public function updateProducts(){
        ...
        foreach($products as $product){
            ...
            try {
                $magentoProduct = $this->productRepository->get($sku,true,0);
                $this->productModel->reset();
                $magentoProduct = $this->productModel->load($magentoProduct->getId());
            }catch(\Exception $ex){
                $magentoProduct = null;
    ...
    private function _createConfigurableProduct($product,$simpleProductsData){
        ...
        // dd($this->storeManager->getStore()->getId());

        //$cProduct = $this->productModel->loadByAttribute('sku', $data['sku']);
        try {
            $cProduct = $this->productRepository->get($data['sku'],true,$store_id);
        }catch(\Exception $ex){
            $cProduct = null;
        }
    ...
    private function _createSimpleProduct($simple){
        ...
        }catch(\Exception $ex){$sProduct = null;}
        if ($sProduct)
    ...
    private function _createSimpleProductForConfigurable($simple,$parent){
        ...
        //$data['name'] = 'TESTOLEG';
        //$data['sku']= $parent['cpid']."-".join("_",$configurableForName);
    ...
    public function saveProduct($product){
        ...
        $ret =  $this->productRepository->save($product);
