...
class Menu	 extends \Magento\Framework\View\Element\Template 
{
	...
	function partition_element(Array $list, $p) {
		...
		foreach($list as $cat => $subcat){
			$childCategory=$subcat->getChildrenCategories()->addIsActiveFilter();
			if(count($childCategory)){
				$target2[] = count($childCategory) + 1;
				$temp = $temp + count($childCategory) + 1;
			}
			else {
				$target2[] = 1;
				$temp = $temp + 1;
		...
		if($temp === count($list)){
			...
			$listlen1 = count($list);
			$partlen1 = floor($listlen1 / $p);
			$listlen2 = $temp;
			$partlen2 = floor($listlen2 / 2);
			$temp2 = 0;
			$temp3 = 0;
			
			foreach($target2 as $key => $tar){
				$temp2 = $temp2 + $tar;
				if(($partlen2/$temp2) > 1){
			...
			$partition[0] = array_slice($list, 0, $temp3 +1);
			$partition[1] = array_slice($list, $temp3 +1,  $listlen1 - $temp3 +1);
	 ...
	 protected function getActiveChildren($parent, $level)
    {
        ...
        if (count($childs))
        {
            ...
            {
				$child2 = $this->_categoryInstance1->create()->load($child->getId())->getIncludeInMenu();
                if ($child->getIsActive() and $child2)
                {
	...
	public function drawColumns($children, $id)
    {
        ...
        //echo 'hahahah'.count($chunks); die($columChunk);
        // --- draw columns ---
        $classSpecial = '';
        $keyLast = 0;
        ...
        foreach ($children as $child)
        {
           // echo "<pre>"; print_r($child->getData()); echo "</pre>";
            if ($child->getIsActive())
            {
				...
				$thumbnail = $this->getThumbUrl($child1->getThumbNail());
                //echo $thumbnail;
                $html.= '<div class="menu-content">';
                if($thumbnail != null) {
					$html.= '<div class="cat-thumbnail"><img width="10" src="'.$thumbnail.'" alt="Thumbnail" /></div>';
				}
